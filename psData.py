...
def get_process_stats() -> str: 
    ...
    Runs the ps command with the formatted output user, group, process id, cpu %, memory %, resident set size, state, start time, execution time and command. 
    If the user or group is longer than 20 characters it is truncated with a '+' and execution time is in seconds. 
    ...
    """
    cmd = "ps axo user:20,group:20,pid,pcpu,pmem,rssize,stat,lstart,etimes,comm".split() 
...
def store_process_stats(ps_list : list) -> pd.DataFrame:
    """Creates a pandas dataframe given a list of processes (each process attributes are in list)
    ...
    """
    cols = ['User', 'Group', 'PID', 'CPU', 'MEM', 'RSS', 'STAT', 'Weekday', 'Month', 'Day', 'Time', 'Year', 'Elapsed', 'Command']
